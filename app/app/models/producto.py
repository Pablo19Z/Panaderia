...
class Producto:
    def __init__(self, id=None, nombre=None, descripcion=None, precio=None, categoria_id=None, stock=0, imagen=None, activo=True, fecha_creacion=None, total_vendido=None):
        self.id = id
        self.nombre = nombre
    ...
    @classmethod
    def get_mas_vendidos(cls, limit=10):
        """Obtiene los productos más vendidos"""
        conn = get_db_connection()
        cursor = conn.cursor()
        
        cursor.execute('''
            SELECT p.*, COALESCE(SUM(dv.cantidad), 0) as total_vendido
            FROM productos p
        ...
        productos = []
        for row in rows:
            producto = cls(*row[:-1])  # Excluir total_vendido del constructor
            producto.total_vendido = row[-1]
