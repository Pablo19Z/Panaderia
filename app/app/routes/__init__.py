...
def register_blueprints(app):
        ...
        if 'user_id' in session:
            try:
                ...
                if cliente:
                    ...
                    print(f"[v0] Favoritos obtenidos de BD: {len(favoritos_data)} items")
                    for favorito in favoritos_data:
                        try:
                            ...
                            producto_obj = type('Producto', (), {
                                ...
                                'descripcion': favorito[7] if len(favorito) > 7 and favorito[7] else 'Delicioso producto de nuestra panadería'  # p.descripcion
                            })()
                            productos_favoritos.append(producto_obj)
                            print(f"[v0] Favorito procesado: {producto_obj.nombre}")
                        except Exception as e:
                            print(f"[v0] Error procesando favorito individual: {e}")
                            continue
                else:
                    print(f"[v0] Cliente no encontrado para user_id: {session['user_id']}")
            except Exception as e:
                print(f"[v0] Error obteniendo favoritos de BD: {e}")
                productos_favoritos = []
        else:
            favoritos_ids = session.get('favoritos', [])
            print(f"[v0] Favoritos en sesión: {favoritos_ids}")
                ...
                for producto_id in favoritos_ids:
                    try:
                        ...
                        if producto:
                            ...
                            print(f"[v0] Favorito de sesión procesado: {producto.nombre}")
                    except Exception as e:
                        print(f"[v0] Error procesando favorito de sesión {producto_id}: {e}")
                        continue
        
        print(f"[v0] Total productos favoritos para mostrar: {len(productos_favoritos)}")
    ...
    @app.route('/recetas')
    @app.route('/recetas/<receta_id>')
    def recetas(receta_id=None):
        from flask import render_template
        return render_template('recetas.html', receta_id=receta_id)
